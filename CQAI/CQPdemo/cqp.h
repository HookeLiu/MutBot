...
// 不知道为啥, Coxxs没把获取群成员列表(160)/获取群列表(161)的API给出来, 在这里手动添加一下, 测试通过
CQAPI(const char *) CQ_getGroupMemberList(int32_t AuthCode, int64_t groupid);  // 160,  //取群成员列表		getGroupMemberList
CQAPI(const char *) CQ_getGroupList(int32_t AuthCode);                         // 161,  //取群列表			getGroupList    
