...
class ChineseWord():
    ...
    def solarTerm(year, month, day):
        a = '小寒 大寒 立春 雨水 惊蛰 春分             清明 谷雨 立夏 小满 芒种 夏至             小暑 大暑 立秋 处暑 白露 秋分             寒露 霜降 立冬 小雪 大雪 冬至'.split()
        return

    def day_lunar(ld):
        a = '初一 初二 初三 初四 初五 初六 初七 初八 初九 初十             十一 十二 十三 十四 十五 十六 十七 十八 十九 廿十             廿一 廿二 廿三 廿四 廿五 廿六 廿七 廿八 廿九 三十'.split()
        return a[ld - 1]
...
def getCalendar_today():
    ...
    twitter =     "今天是" + str(solar_year) + "年" + str(solar_month) + "月" + str(solar_day) + "日" + " "     + ChineseWord.weekday_str(solar_weekday) + " 农历" + ChineseWord.year_lunar(lunar_year)     + ChineseWord.month_lunar(lunar_isLeapMonth,lunar_month)     + ChineseWord.day_lunar(lunar_day) + festival
    print(twitter)
    return twitter
...
def getLunar_today():
    solar = SolarDate()
    LunarDate.fromSolarDate(solar_year, solar_month, solar_day)
    twitter = ChineseWord.year_lunar(lunar_year) + ChineseWord.month_lunar(lunar_isLeapMonth,lunar_month) + ChineseWord.day_lunar(lunar_day)
    return twitter
...
def getCalendar_all_day():
    ...
    for solar_month in range(1, 13):
        ...
        for solar_day in range(1, solar_day_max + 1):
            ...
            twitter = ("message(" + str(index_yyddmm) + ') = "') +             str(solar_year) + "年" + str(solar_month) + "月" + str(solar_day) + "日" + " "             + ChineseWord.weekday_str(solar_weekday) + " 农历" + ChineseWord.year_lunar(lunar_year)             + ChineseWord.month_lunar(lunar_isLeapMonth,lunar_month)             + ChineseWord.day_lunar(lunar_day) + festival + '"'
            print(twitter)
